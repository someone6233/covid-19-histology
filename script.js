...
/* ---------------------------------------------------------------
   3b. Label-chip helper (buttons under a real photo/illustration —
       no pixel coordinates needed, so nothing can be "in the wrong
       place" the way an SVG pin over a photo could be)
---------------------------------------------------------------- */
function wireLabelSpots(spotDefs, noteEl, nextBtn) {
  let seenCount = 0;
  const total = spotDefs.length;
  spotDefs.forEach(def => {
    ...
    btn.addEventListener("click", () => {
      Sound.click();
      if (!btn.classList.contains("seen")) {
...
function setupCellScreen() {
  const spots = [
    { id: "hs-membrane", title: "Cell membrane", text: "A thin, flexible boundary that controls what goes in and out of the cell. This is exactly what the virus's spike protein tries to unlock." },
    { id: "hs-nucleus", title: "Nucleus", text: "The cell's control centre — it stores the genetic instructions (DNA) and directs everything the cell does." },
    { id: "hs-mito", title: "Mitochondria", text: "Often called the 'powerhouse' of the cell — they turn nutrients into usable energy so the cell can do its work." },
    { id: "hs-cytoplasm", title: "Cytoplasm", text: "The jelly-like fluid filling the inside of the cell, where the organelles sit and many chemical reactions happen." }
  ];
...
function setupExamineScreen() {
  ...
  wireLabelSpots(spots, document.getElementById("examine-note"), document.getElementById("examine-next"));
}

/* ---------------------------------------------------------------
   10b. Compare two real slides (normal vs. COVID-19 lung)
---------------------------------------------------------------- */
function setupCompareScreen() {
  const slideA = document.getElementById("compare-slide-a-btn");
  const slideB = document.getElementById("compare-slide-b-btn");
  const feedback = document.getElementById("compare-pick-feedback");
  const reveal = document.getElementById("compare-reveal");
  const captionA = document.getElementById("compare-a-caption");
  const captionB = document.getElementById("compare-b-caption");
  let answered = false;

  function pick(isCorrect, chosenBtn, otherBtn) {
    if (answered) return;
    answered = true;
    Sound.click();
    ...
    if (isCorrect) {
      ...
      Sound.correct();
      recordTally("compare-pick", 1, 1);
    } else {
      chosenBtn.classList.add("incorrect");
      otherBtn.classList.add("correct");
      Sound.wrong();
      recordTally("compare-pick", 0, 1);
    }
    captionA.textContent = "Slide A — Normal lung tissue";
    captionB.textContent = "Slide B — Patient's lung tissue (diffuse alveolar damage)";
    feedback.innerHTML = "<strong>Slide B is the infected one.</strong> Its air sacs are lined with a pink, glassy layer (a hyaline membrane) and the walls look thickened and crowded with debris and inflammatory cells. Slide A's air spaces stay clear and open.";
    reveal.style.display = "block";
  }

  slideA.addEventListener("click", () => pick(false, slideA, slideB));
  slideB.addEventListener("click", () => pick(true, slideB, slideA));

  const questions = [
    { prompt: "What lines the alveoli in the damaged slide that isn't present in the normal slide?", options: ["A thin, glassy hyaline membrane made of dead cells and protein", "Extra bone tissue", "Nothing — the slides are identical", "Hair follicles"], correct: 0, explain: "That glassy pink lining is the hyaline membrane — a hallmark of diffuse alveolar damage, the pattern of injury found in severe COVID-19 lungs." },
    { prompt: "What does this damage do to gas exchange?", options: ["Makes it easier for oxygen to cross into the blood", "Makes it harder for oxygen to cross into the blood", "Has no effect on oxygen at all"], correct: 1, explain: "A thickened, debris-filled wall is a much bigger barrier for oxygen to cross, which is why patients become short of breath." }
  ];
  renderQuiz(document.getElementById("compare-quiz"), questions, "compare-quiz", () => {
    document.getElementById("compare-next").disabled = false;
  });
}
...
// Safety net: the video can finish before every timed checkpoint has been
// reached (e.g. if the real video is shorter than a checkpoint's timestamp,
// or the viewer scrubs the seek bar). Without this, "Next: run lab tests"
// would stay disabled forever because checkVideoDone() never sees all
// checkpoints as done. When the video ends, ask any leftover checkpoint
// questions in one go, then unlock the button no matter what.
function handleVideoEnded() {
  if (videoState.pollTimer) clearInterval(videoState.pollTimer);

  const remaining = VIDEO_CHECKPOINTS
    .map((cp, i) => ({ cp, i }))
    .filter(({ i }) => !videoState.doneCheckpoints.has(i));
  ...
  document.getElementById("vq-title").textContent = "The video's done — just a couple more questions";
  body.innerHTML = "";
  const wrap = document.createElement("div");
  body.appendChild(wrap);

  renderQuiz(wrap, remaining.map(({ cp }) => ({
    prompt: cp.prompt, options: cp.options, correct: cp.correct, explain: cp.explain
  })), "video", () => {
    remaining.forEach(({ i }) => videoState.doneCheckpoints.add(i));
